...
for file_no in range(csv_number[0], csv_number[1] + 1):
    ...
    # download_dir = "{folder directory in your pc}" + "\\StarData_" + str(file_no) + ".csv"
    # Note that you have to only change the text in the {} brackets, below is an example of my download_dir
    download_dir = "C:\\Users\\advi\\Downloads" + "\\StarData_" + str(file_no) + ".csv"
    ...
    df = pd.read_csv(download_dir, skiprows=4)
    ...
    for i in range(mid_break, df.shape[0] if file_no == csv_number[0] else df.shape[0]):
        ...
        # This is the program to analyse each subset data a particular "starstr"
        for row in range(rows):

            try:
                file1 = initial[row].download(quality_bitmask="default").remove_nans()
                temporary_file = file1.remove_outliers(sigma=6).flatten()
            except:
